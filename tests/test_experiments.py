"""
Tests for A/B Experiment API Endpoints
Test experiment creation, execution, and analysis.
"""
import pytest
...
class TestExperimentHealthCheck:
    """Test experiment service health check"""

    def test_experiment_health_check(self, client: TestClient):
        """Test experiment health endpoint"""
...
class TestExperimentCRUD:
    ...
    def test_create_experiment(self, client: TestClient):
        ...
        experiment_data = {
            "name": "Test Promotion A vs B",
            "description": "Compare 10% vs 15% discount",
            "experiment_type": "promotion_comparison",
            "control_config": {
                "name": "10% Off",
...
class TestExperimentExecution:
    """Test experiment execution and variant assignment"""
    ...
    def test_run_experiment(self, client: TestClient):
        """Test running an experiment and getting variant assignment"""
        product_response = client.post("/products/", json={
            "sku": "RUN-EXP-001",
            "title": "Run Experiment Product",
            "base_price": 2000.0,
            "stock": 50,
            "tax_rate": 12.0
        })
        ...
        response = client.post(
            ...
            params={"product_id": product_id, "quantity": 5}
        )
    ...
    def test_run_inactive_experiment(self, client: TestClient):
        """Test running an inactive experiment"""
        product_response = client.post("/products/", json={
            "sku": "INACTIVE-EXP-001",
            "title": "Inactive Experiment Product",
            "base_price": 1500.0,
            "stock": 30,
            "tax_rate": 18.0
        })
...
class TestExperimentResults:
    """Test experiment result collection and analysis"""

    def test_get_experiment_results_no_data(self, client: TestClient):
        """Test getting results for experiment with no data"""
        create_response = client.post("/experiments/", json={
            "name": "No Data Experiment",
            "control_config": {"name": "Control"},
            "variant_config": {"name": "Variant"}
        })
        experiment_id = create_response.json()["id"]
    ...
    def test_get_experiment_results_with_data(self, client: TestClient):
        """Test getting results after running experiment"""
        product_response = client.post("/products/", json={
            "sku": "RESULTS-EXP-001",
            "title": "Results Experiment Product",
            "base_price": 3000.0,
            "stock": 100,
            "tax_rate": 18.0
        })
...
class TestShadowEvaluation:
    """Test shadow evaluation (running both variants in parallel)"""

    def test_shadow_evaluation_price_comparison(self, client: TestClient):
        """Test that shadow evaluation shows both variant results"""
        product_response = client.post("/products/", json={
            "sku": "SHADOW-001",
            "title": "Shadow Eval Product",
            "base_price": 5000.0,
            "stock": 50,
            "tax_rate": 18.0
        })
...
class TestTrafficSplit:
    ...
    def test_traffic_split_distribution(self, client: TestClient):
        """Test that traffic is split according to configuration"""
        product_response = client.post("/products/", json={
            "sku": "SPLIT-001",
...
class TestExperimentIntegration:
    ...
    def test_complete_experiment_workflow(self, client: TestClient):
        ...
        create_response = client.post("/experiments/", json={
            ...
            "control_config": {
                "name": "Control Flat ₹200",
                "discount_type": "flat",
                "discount_value": 200.0
            },
            "variant_config": {
                "name": "Variant Flat ₹400",
                "discount_type": "flat",
