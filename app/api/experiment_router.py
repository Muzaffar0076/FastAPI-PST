...
def list_experiments(skip: int=Query(0, ge=0), limit: int=Query(100, ge=1, le=1000), db: Session=Depends(get_db)):
...
def run_experiment(experiment_id: int, product_id: int, quantity: int=1, target_currency: Optional[str]=None, include_tax: Optional[bool]=None, db: Session=Depends(get_db)):
    result = experiment_service.run_experiment(db, experiment_id, product_id, quantity, target_currency, include_tax)
    if not result:
        raise HTTPException(status_code=404, detail='Experiment not found')
    if 'error' in result:
...
def experiment_health():
    return {'service': 'Experiment Engine', 'status': 'operational', 'features': ['A/B Testing', 'Shadow Evaluation', 'Traffic Splitting', 'Result Analysis']}