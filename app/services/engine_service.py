...
def calculate_price_with_explanation(db: Session, product_id: int, quantity: int, target_currency: Optional[str]=None, include_tax: Optional[bool]=None, rounding_strategy: str='half_up', enable_audit: bool=True, user_id: Optional[str]=None, ip_address: Optional[str]=None, user_agent: Optional[str]=None, request_id: Optional[str]=None) -> Optional[Dict[str, Any]]:
    cache_key = CacheService._get_key('price', product_id, quantity, target_currency or 'default', include_tax if include_tax is not None else 'default', rounding_strategy)
    ...
    promos = db.query(Promotion).filter(Promotion.is_active == True).filter((Promotion.product_id == product_id) | (Promotion.applies_to_category == True)).order_by(Promotion.priority.asc()).all()
    explanation = []
    ...
    for promo in promos:
        ...
        if discount > 0:
            if promo.stacking_enabled:
                ...
                applied_promotions.append({'name': promo.name, 'discount': float(discount), 'reason': reason, 'priority': promo.priority})
                explanation.append(f'Rule Applied (Stacked): {promo.name} - {reason} (Priority: {promo.priority})')
            elif discount > total_discount:
                total_discount = discount
                current_price = base_price - total_discount
                applied_promotions = [{'name': promo.name, 'discount': float(discount), 'reason': reason, 'priority': promo.priority}]
    ...
    result = {'original_price': float(round_price(original_converted, rounding_strategy)), 'base_price_after_discount': float(round_price(base_amount_converted, rounding_strategy)), 'tax_amount': float(round_price(tax_amount_converted, rounding_strategy)), 'final_price': float(final_price), 'discount_amount': float(round_price(discount_converted, rounding_strategy)), 'applied_promotion': primary_promotion, 'applied_promotions': applied_promotions, 'currency': display_currency, 'tax_rate': float(tax_rate), 'tax_inclusive': tax_inclusive, 'explanation': explanation, 'cached': False}
    CacheService.set(cache_key, result, ttl=3600)
    if enable_audit and (not cached_result):
        try:
            AuditService.log_price_calculation(db=db, product_id=product_id, quantity=quantity, pricing_result=result, user_id=user_id, ip_address=ip_address, user_agent=user_agent, request_id=request_id)
        except Exception:
            pass
    return result